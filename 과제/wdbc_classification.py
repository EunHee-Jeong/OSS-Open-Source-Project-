...
def model_training(model, wdbc, name):
...
if __name__ == '__main__':
    ...
    # Train a model
    svm_clf = svm.SVC()
    svm_clf = linear_model.SGDClassifier()                       # Mission #2) Try at least two different classifiers

    #model.fit(wdbc.data, wdbc.target)

    model_training(svm_clf, wdbc, "svm.SVC")
    model_training(sgd_clf, wdbc, "SGD")

    # plt.show()
    # plt.savefig('wdbc_classification_svm.png')
    # plt.savefig('wdbc_classification_SGD.png')
